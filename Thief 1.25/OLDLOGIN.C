...
BOOL CALLBACK OldLoginBoxWndProc (HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
    {
    static HWND hServer, hHelper, hHandle, hPassword, hScript ;
    FARPROC LpProc ;
    char cHelper [_MAX_PATH], cHandle [_MAX_PATH], cPassword [_MAX_PATH], cScript [_MAX_PATH] ;
    ...
    switch (iMsg)
        ...
        case WM_INITDIALOG :
            ...
            if (System.bQuickConnection)
                {
                strcpy (cTmp, "Quick Connection") ;
                }
            else
            if (System.bCDROMConnection)
                {
                sprintf (cTmp, "CDROM Connection: %s", Login.cLoginName) ;
                }
            else
                {
                sprintf (cTmp, "Connection: %s", Login.cLoginName) ;
                }
            ...
            switch (Login.nLoginType)
                {
                case SERVER_FICS :    strcpy (cTmp1, "fics") ;  break ;
                case SERVER_ICC  :    strcpy (cTmp1, "icc") ;   break ;
                case SERVER_NONFICS : strcpy (cTmp1, "other") ; break ;
                default :             strcpy (cTmp1, "fics") ;  break ;
                }
            sprintf (cTmp, "%s %s [%s] (%s)", Login.cLoginAddress, Login.cLoginPort, Login.cLoginPrompt, cTmp1) ;
            ...
            SendDlgItemMessage (hwnd, IDD_HELPER,   WM_SETTEXT, (WPARAM) strlen (Login.cLoginScript),   (LPARAM) Login.cLoginHelper) ;
            SendDlgItemMessage (hwnd, IDD_HANDLE,   WM_SETTEXT, (WPARAM) strlen (Login.cLoginHandle),   (LPARAM) Login.cLoginHandle) ;
            SendDlgItemMessage (hwnd, IDD_PASSWORD, WM_SETTEXT, (WPARAM) strlen (Login.cLoginPassword), (LPARAM) Login.cLoginPassword) ;
            SendDlgItemMessage (hwnd, IDD_SCRIPT,   WM_SETTEXT, (WPARAM) strlen (Login.cLoginScript),   (LPARAM) Login.cLoginScript) ;
